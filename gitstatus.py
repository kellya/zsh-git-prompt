...
if 'not a git repository' in error_string:
    sys.exit(0)
...
nb_untracked = len([0 for status in Popen(
    [
        'git',
        'status',
        '--porcelain',
    ...
    stdout=PIPE
).communicate()[0].decode("utf-8").splitlines() if status.startswith('??')])
untracked = str(nb_untracked)

ahead, behind = 0, 0

if not branch:  # not on any branch
    branch = prehash + Popen(
        [
            'git',
            'rev-parse',
            '--short',
